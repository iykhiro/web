...
const tituloOriginal = 'ウェブサイト...';
const tituloElemento = document.getElementById('titlesite');
let tituloIndex = 1;
let apagando = false;
...
function animarTitulo() {
  if (!apagando) {
    tituloIndex++;
    tituloElemento.textContent = tituloOriginal.slice(0, tituloIndex);

    if (tituloIndex >= tituloOriginal.length) {
      apagando = true;
  ...
  setTimeout(animarTitulo, apagando ? 120 : 180);
}

tituloElemento.textContent = tituloOriginal.slice(0, 1);
animarTitulo();